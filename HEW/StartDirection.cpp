	...
	, m_startDraw(false)
{
	...
	m_tSize[0] = { SCREEN_WIDTH * 0.3f,SCREEN_HEIGHT * 0.09f, 0.0f };
	...
	m_tSize[1] = { SCREEN_WIDTH*0.07f,SCREEN_HEIGHT*0.1f, 1.0f };
	m_tSize[2] = { SCREEN_WIDTH*0.07f,SCREEN_HEIGHT*0.1f, 1.0f };
	m_tSize[3] = { SCREEN_WIDTH*0.07f,SCREEN_HEIGHT*0.1f, 1.0f };
	m_tSize[4] = { SCREEN_WIDTH*0.07f,SCREEN_HEIGHT*0.1f, 1.0f };
	m_tSize[5] = { SCREEN_WIDTH*0.07f,SCREEN_HEIGHT*0.1f, 1.0f };
	m_tSize[6] = { SCREEN_WIDTH*0.07f,SCREEN_HEIGHT*0.1f, 1.0f };
	//出現時間の初期化
	for (int i = 0; i < 7; i++)
	{
		m_CountTimer[i] = 0.0f;
...
void CStartDirection::Draw()
{
	if (m_NowDraw)
	{
		...
		if (m_startDraw && m_CountTimer[0] < m_OldCountTimer[1] * (3.0f / 4.0f))
		{
			...
			m_StratSprite[0]->SetRotation(0.0f, TORAD(180.0f),0.0f);
