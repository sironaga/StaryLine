...
void COption::DrawSound()
{
	...
	Sprite::Draw();	
	m_pParam[BAR_GAGE]->size.x = 590.0f / 10.0f * m_nTempValue[MASTER] * m_fMul;
	m_pParam[BAR_GAGE]->pos = { (260.0f * m_fMul - (m_nTempValue[MASTER] / 10.0f) - m_pParam[BAR_GAGE]->size.x / 2.0f) + MovePos.X,(300.0f * m_fMul) + MovePos.Y };
	...
	Sprite::Draw();
	m_pParam[BAR_HANDLE]->pos = { ((m_nTempValue[MASTER] * 55.0f - 70.0f) * m_fMul) + MovePos.X ,(300.0f * m_fMul) + MovePos.Y };
