...
Field::Field(StageType StageNum)
{
		...
		case (int)EStageType::STAGE_GRASSLAND:
			if (!m_pModel->Load(MODEL_PASS("Stage/Stage01_.fbx"), 1.0f/*倍率指定 */, Model::None/*反転設定*/))MessageBox(NULL, "Ground", "Error", MB_OK);aster
			break;
		case (int)EStageType::STAGE_DESERT:
			if (!m_pModel->Load(MODEL_PASS("Stage/Stage02_Desert.fbx"), 1.0f/*倍率指定 */, Model::None/*反転設定*/))MessageBox(NULL, "Ground", "Error", MB_OK);
			break;
		case (int)EStageType::STAGE_SNOWFIELD:
			if (!m_pModel->Load(MODEL_PASS("Stage/Stage03_.fbx"), 1.0f/*倍率指定 */, Model::None/*反転設定*/))MessageBox(NULL, "Ground", "Error", MB_OK);
			break;
		}
