	...
	:m_Field_Model{}
{
	...
	for (int i = 0; i < MAX_MODEL; i++)
	{
		switch (i)
		{
		case TILELOW_FOREST:if (!m_pModel[i]->Load("Assets/Model/Stage/Stage02_Desert.fbx", 1.0f/*倍率指定 */, Model::XFlip/*反転設定*/))MessageBox(NULL, "Ground", "Error", MB_OK);
		//case TILELOW_FOREST:if (!m_pModel[i]->Load("Assets/Model/LowPolyNature/Rock_02.fbx", 0.01f/*倍率指定 */, Model::XFlip/*反転設定*/))MessageBox(NULL, "Ground", "Error", MB_OK);
			break;
		default:
			break;
		}
...
void Field::FieldModelInit(FieldModel* InModel, int MaxModel)
{
	...
	{
		pModel->m_Pos = DirectX::XMFLOAT3(-60.0f, -12.0f, 40.0f);
		pModel->m_Angle = DirectX::XMFLOAT3(TORAD(-5), TORAD(180), TORAD(0));
