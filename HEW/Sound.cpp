...
CSound::CSound(const std::wstring& fileName)
{
	...
	if (FAILED(result)) printf("COMの初期化失敗");
	result = XAudio2Create(&m_pXAudio2);
	if (FAILED(result)) printf("m_pXAudio2の初期化失敗");

	if (m_pXAudio2)
	{
		result = m_pXAudio2->CreateMasteringVoice(&m_pMasteringVoice);
	}
	if (FAILED(result)) printf("m_pMasteringVoiceの初期化失敗");
	if (!LoadWaveFile(fileName, &waveData))cout<<"ファイルの読み込み失敗";
}
